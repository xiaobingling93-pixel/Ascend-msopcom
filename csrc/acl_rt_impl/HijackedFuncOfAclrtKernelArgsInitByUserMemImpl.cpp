...
aclError HijackedFuncOfAclrtKernelArgsInitByUserMemImpl::Post(aclError ret)
{
    if (ret != ACL_SUCCESS || funcHandle_ == nullptr || argsHandle_ == nullptr) {
        return ret;
    }
    auto ctx = ArgsManager::Instance().CreateContext(funcHandle_, argsHandle_);
    if (!ctx) {
        DEBUG_LOG("Create args context failed on aclrtKernelArgsInitByUserMemImpl");
    }
    return ret;
}
