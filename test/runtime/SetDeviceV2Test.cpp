...
TEST(rtSetDeviceV2, call_pre_function_with_valid_device_id_expect_return)
{
    MOCKER(IsOpProf).stubs().will(returnValue(false));
    HijackedFuncOfSetDeviceV2 instance;
    instance.Pre(1, RT_DEVICE_MODE_SINGLE_DIE);
    EXPECT_EQ(DeviceContext::Local().GetDeviceId(), 1);
    instance.Call(1, RT_DEVICE_MODE_SINGLE_DIE);
}

TEST(rtSetDeviceV2, opprof_call_with_originfunc)
{
    ...
    auto func = [](int32_t, rtDeviceMode) ->
